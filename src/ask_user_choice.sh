...
read_categories() {
	...
	if [[ $data_source == "selected" ]]; then
		cmd "$(awk '/installationType:/ {print $2}' "$SELECTED_SOFTWARE_LIST_LOCATION")"
	fi
	if [[ $data_source == "supported" ]]; then
		cmd "$(awk '/installationType:/ {print $2}' "$SUPPORTED_SOFTWARE_LIST_LOCATION")"
	fi
...
read_software_packages_per_category() {
	...
	if [ "$data_source" == "selected" ]; then
		cmd "$(awk '/'"$installation_type"':/ {print $2}' "$SELECTED_SOFTWARE_LIST_LOCATION")"
	fi
	if [ "$data_source" == "supported" ]; then
		cmd "$(awk '/'"$installation_type"':/ {print $2}' "$SUPPORTED_SOFTWARE_LIST_LOCATION")"
	fi
}
	...
	if [ ${#software_package} -ge 1 ]; then
		while true; do
			if [ "$activity" == "install" ]; then
				read -rp "Do you wish to install: $software_package?" yn
			elif [ "$activity" == "uninstall" ]; then
				read -rp "Do you wish to uninstall: $software_package?" yn
		    fi
		    case $yn in
			[Yy]* ) echo "true"; break;;
			[Nn]* ) echo "false"; break;;
